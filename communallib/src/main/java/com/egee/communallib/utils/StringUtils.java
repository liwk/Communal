...
public class StringUtils {
     ...
     * http://api-test.qezhuan.com/newer-tutorials?rightTitle='规则'&rightUrl='http://www.baidu.com'
     *
     * @param url
     */
     ...
     * @return
     */
    public static String moreCountEllipsize(String content, int maxLen) {
     ...
     * @return 转换后的数字
     */
    public static CharSequence numberConversion(String value, int scale) {
        if (TextUtils.isEmpty(value)) {
            return "0";
        }
        if (TextUtils.isDigitsOnly(value)) {
            if (value.length() > 4) {
                return TextUtils.concat(new BigDecimal(value).divide(new BigDecimal(10000), scale, BigDecimal.ROUND_HALF_UP).toString(), "W");
            } else {
                return value;
            }
        } else {
     ...
     * @return
     */
    public static String getNotEmptyMoney(String money) {
        if (TextUtils.isEmpty(money)) {
            return ContextUtils.getContext().getResources().getString(R.string.zero_amount);
        }
        return money;
    }
